const messages = {
    brandSubtitle: "Локальний пошук файлів",
    minimizeWindow: "Згорнути вікно",
    maximizeWindow: "Розгорнути або відновити вікно",
    closeWindow: "Закрити вікно",
    languageLabel: "Мова",
    rootLabel: "Папка пошуку",
    rootPlaceholder: "Виберіть або введіть шлях до папки",
    pickDirectory: "Вибрати папку",
    queryLabel: "Ключове слово імені файлу",
    queryPlaceholder: "Наприклад report, .rs, рахунок",
    regexPlaceholder: "Наприклад ^report_\\d{4}\\.pdf$",
    caseSensitive: "Враховувати регістр",
    useRegex: "Регулярний вираз",
    includeHidden: "Включати приховані файли",
    searchButton: "Шукати",
    eyebrow: "Пошук",
    waiting: "Готово до пошуку",
    statFiles: "Проскановано файлів",
    statDirs: "Проскановано папок",
    statTime: "Час",
    emptyTitle: "Виберіть папку для перегляду або введіть ключове слово назви файлу",
    emptyText: "Пошук виконується локально. Імена, шляхи і вміст не завантажуються.",
    emptyFolderTitle: "Папка порожня",
    emptyFolderText: "У вибраній папці немає видимих елементів.",
    searching: "Пошук...",
    found: (count) => `Знайдено результатів: ${count}`,
    noResultsTitle: "Збігів немає",
    noResultsText: "Спробуйте інше ключове слово або включіть приховані файли.",
    searchFailed: "Помилка пошуку",
    cannotComplete: "Не вдалося завершити пошук",
    openFailed: "Не вдалося відкрити",
    file: "Файл",
    directory: "Папка",
    expandFolder: "Розгорнути папку",
    collapseFolder: "Згорнути папку",
    unknownTime: "Невідомий час",
    invalidDirectory: "Вибрана папка не існує або не є папкою.",
    invalidRegex: "Регулярний вираз недійсний.",
    searchTaskFailed: "Завдання пошуку завершилося помилкою.",
    openTaskFailed: "Завдання відкриття завершилося помилкою.",
    openPathFailed: "Не вдалося відкрити цей шлях.",
    updateEyebrow: "Оновлення",
    updateTitle: "Оновлення ПЗ",
    checkUpdateButton: "Перевірити",
    checkingUpdate: "Перевірка оновлень...",
    updateIdle: "Перевірити останню версію в GitHub Releases.",
    updateAvailable: (version) => `Доступна версія ${version}.`,
    upToDate: "Ви використовуєте останню версію.",
    currentVersion: "Поточна",
    latestVersion: "Остання",
    publishedAt: "Опубліковано",
    releaseNotes: "Журнал змін",
    openReleaseButton: "Відкрити реліз",
    emptyReleaseNotes: "Цей реліз не містить опису.",
    updateCheckFailed: "Не вдалося перевірити оновлення.",
    versionCompareFailed: "Не вдалося порівняти версії.",
    packageChecksums: "Контрольні суми пакетів",
    noPackageChecksums: "GitHub не повернув метадані контрольних сум пакетів.",
    updateRateLimited: "Досягнуто ліміт звернень до GitHub. Спробуйте пізніше.",
    updateNotFound: "Випуск GitHub не знайдено.",
    updateNetworkFailed: "Помилка мережевого запиту.",
    cancelSearchButton: "Скасувати",
    statSkipped: "Пропущено",
    resultFilterLabel: "Фільтр результатів",
    resultFilterPlaceholder: "Фільтрувати видимі результати",
    typeFilterLabel: "Тип",
    typeAll: "Усі",
    typeFiles: "Файли",
    typeFolders: "Папки",
    sortLabel: "Сортування",
    sortNameAsc: "Назва А-Я",
    sortNameDesc: "Назва Я-А",
    sortModifiedDesc: "Спочатку нові",
    sortModifiedAsc: "Спочатку старі",
    sortSizeDesc: "Спочатку великі",
    sortSizeAsc: "Спочатку малі",
    progressIdle: "Перебіг пошуку з'явиться під час сканування.",
    scanningProgress: (matches, path) => "Сканування... Збігів: " + matches + (path ? " · " + path : ""),
    searchCancelled: "Пошук скасовано",
    noVisibleResultsTitle: "Немає видимих результатів",
    noVisibleResultsText: "Змініть фільтри або сортування.",
    visibleResultCount: (visible, total) => "Показано " + visible + " із " + total,
    openUpdates: "Перевірити оновлення",
    openSettings: "Відкрити налаштування",
    closeDialog: "Закрити вікно",
    preferencesEyebrow: "Параметри",
    settingsTitle: "Налаштування",
    settingsCategories: "Категорії налаштувань",
    appearanceCategory: "Вигляд",
    languageDescription: "Виберіть мову інтерфейсу.",
    themeTitle: "Тема",
    themeDescription: "Виберіть готовий вигляд FileNavigation.",
    lightTheme: "Світла",
    darkTheme: "Темна",
    automaticUpdateButton: "Завантажити, встановити й перезапустити",
    preparingUpdate: "Визначення системи та підготовка оновлення...",
    updateDownloading: (percent) => "Завантаження оновлення... " + percent + "%",
    installingUpdate: "Завантаження завершено. Встановлення...",
    restartingUpdate: "Встановлення завершено. Перезапуск...",
    automaticUpdateFailed: "Не вдалося виконати автоматичне оновлення.",
    updaterUnavailable: "Для цієї системи немає сумісного пакета оновлення.",
    toolsLabel: "Інструменти", toolSearch: "Пошук", toolIntegrity: "Цілісність",
    integritySourceLabel: "Джерело", integritySourceFile: "Один файл", integritySourceFolder: "Уся папка", integritySourceCurrent: "Поточна папка пошуку", integritySourceFiltered: "Відфільтровані результати",
    integrityTargetLabel: "Файл або папка", integrityTargetPlaceholder: "Виберіть файл або папку", pickIntegrityTarget: "Вибрати файл або папку", pickIntegrityFolder: "Вибрати папку для перевірки", pickIntegrityFile: "Вибрати файл для перевірки",
    integrityAlgorithmLabel: "Алгоритм", expectedHashLabel: "Очікуваний відбиток (необов’язково)", expectedHashPlaceholder: "Вставте попередній відбиток для порівняння", verifyIntegrityButton: "Перевірити",
    integrityEyebrow: "Цілісність", integrityReady: "Готово до перевірки", integrityFiles: "Файли", integrityBytes: "Прочитано",
    integrityProgressIdle: "Виберіть джерело для обчислення відбитка.", integrityEmptyTitle: "Локальна перевірка файлів", integrityEmptyText: "Обчисліть відбиток файла, папки або відфільтрованих результатів.",
    integrityReading: "Обчислення відбитків", integrityComplete: "Перевірку завершено", integrityCompleteDetail: "Усі файли прочитано, загальний відбиток готовий.",
    integrityCancelled: "Перевірку скасовано", integrityCancelledDetail: "Операцію зупинено до перевірки всіх файлів.", integrityFailed: "Помилка перевірки",
    integrityCompleteTitle: "Відбиток обчислено", integrityCompleteText: "Збережіть загальний відбиток для повторної перевірки набору.",
    integrityMatchTitle: "Цілісність підтверджено", integrityMatchText: "Обчислений відбиток збігається з очікуваним.", integrityMismatchTitle: "Цілісність не збігається", integrityMismatchText: "Вміст, шляхи або набір файлів відрізняються від очікуваного відбитка.",
    aggregateFingerprint: "Загальний відбиток", copyFingerprint: "Копіювати відбиток", fingerprintCopied: "Відбиток скопійовано.", copyFailed: "Не вдалося скопіювати відбиток.", fileFingerprints: "Відбитки файлів",
    invalidFile: "Виберіть коректний файл.", invalidExpectedHash: "Очікуваний відбиток некоректний для вибраного алгоритму.", unsupportedHashAlgorithm: "Алгоритм хешування не підтримується.",
    integrityTaskFailed: "Не вдалося завершити перевірку цілісності.", fileReadFailed: "Не вдалося прочитати файл.", fileChangedDuringCheck: "Файл змінився під час перевірки. Запустіть її знову.",
    folderReadFailed: "Не вдалося повністю прочитати папку. Відбиток не створено.", noFilteredFiles: "У відфільтрованих результатах немає файлів.", invalidIntegritySource: "Виберіть коректне джерело."
    ,toolDuplicates: "Дублікати", duplicatesSourceLabel: "Джерело", duplicatesSourceFolder: "Вибрати папку", duplicatesSourceCurrent: "Поточна папка пошуку", duplicatesSourceFiltered: "Відфільтровані результати", duplicatesFolderLabel: "Папка", duplicatesFolderPlaceholder: "Виберіть папку для сканування", pickDuplicatesFolder: "Вибрати папку", duplicatesSafetyNote: "Файли порівнюються за SHA-256. Видалені копії потрапляють до системного кошика.", scanDuplicatesButton: "Сканувати дублікати",
    duplicatesEyebrow: "Дублікати файлів", duplicatesReady: "Готово до сканування", duplicateGroups: "Групи", duplicateCopies: "Зайві копії", reclaimableSpace: "Можна звільнити", duplicatesProgressIdle: "Виберіть джерело для пошуку однакових файлів.", duplicatesEmptyTitle: "Безпечно знайдіть дублікати", duplicatesEmptyText: "Групуються лише побайтово однакові файли. Нічого не вибирається і не видаляється автоматично.", selectedCopies: "вибрано", autoSelectDuplicates: "Вибрати зайві копії", moveSelectedToTrash: "Перемістити в кошик",
    duplicatesConfirmEyebrow: "Підтвердження очищення", duplicatesConfirmTitle: "Перемістити вибрані копії в кошик?", duplicatesConfirmSafety: "У кожній групі завжди зберігається один перевірений файл.", cancelAction: "Скасувати", confirmMoveToTrash: "Перемістити в кошик", duplicatesConfirmDetail: (count, size) => `Перемістити ${count} вибраних файлів (${size}) до системного кошика?`, duplicatesScanning: "Сканування дублікатів", duplicatesHashing: "Порівняння вмісту", duplicatesComplete: "Сканування завершено", duplicatesCompleteDetail: (groups, size) => `Знайдено груп: ${groups} · можна звільнити ${size}`, duplicatesNoMatches: "Дублікатів не знайдено", duplicatesNoMatchesDetail: "У цьому джерелі немає побайтово однакових файлів.", duplicatesCancelled: "Сканування скасовано", duplicatesCancelledDetail: "Сканування зупинено до порівняння всіх кандидатів.", duplicatesFailed: "Помилка сканування дублікатів",
    duplicateGroupTitle: (index) => `Група ${index}`, keptCopy: "Зберегти", openItem: "Відкрити файл", duplicateKeepRequired: "У кожній групі має залишитися хоча б один файл.", duplicateTaskFailed: "Не вдалося завершити сканування.", invalidDuplicateSource: "Виберіть правильне джерело.", duplicateCancelled: "Сканування дублікатів скасовано.", duplicateSelectionChanged: "Вибрані файли змінилися або більше не однакові. Проскануйте знову.", noDuplicateSelection: "Виберіть хоча б одну копію.", duplicateDeleteFailed: "Не вдалося перемістити файли в кошик.", duplicatesDeleteComplete: "Очищення завершено", duplicatesDeleteCompleteDetail: "Вибрані копії переміщено до кошика.", duplicatesDeletedDetail: (count, size) => `Переміщено файлів: ${count} · звільнено ${size}`, duplicatesDeletePartial: "Очищення часткове", duplicatesDeletePartialDetail: (deleted, failed) => `Переміщено: ${deleted}; не вдалося перемістити: ${failed}.`
};

export default messages;
