const messages = {
    brandSubtitle: "Lokalne wyszukiwanie plików",
    minimizeWindow: "Minimalizuj okno",
    maximizeWindow: "Maksymalizuj lub przywróć okno",
    closeWindow: "Zamknij okno",
    languageLabel: "Język",
    rootLabel: "Folder wyszukiwania",
    rootPlaceholder: "Wybierz lub wpisz ścieżkę folderu",
    pickDirectory: "Wybierz folder",
    queryLabel: "Słowo kluczowe nazwy pliku",
    queryPlaceholder: "Na przykład report, .rs, faktura",
    regexPlaceholder: "Na przykład ^report_\\d{4}\\.pdf$",
    caseSensitive: "Rozróżniaj wielkość liter",
    useRegex: "Wyrażenie regularne",
    includeHidden: "Uwzględnij ukryte pliki",
    searchButton: "Szukaj",
    eyebrow: "Szukaj",
    waiting: "Gotowe do wyszukiwania",
    statFiles: "Przeskanowane pliki",
    statDirs: "Przeskanowane foldery",
    statTime: "Czas",
    emptyTitle: "Wybierz folder do przeglądania lub wpisz słowo kluczowe nazwy pliku",
    emptyText: "Wyszukiwanie działa lokalnie. Nazwy, ścieżki i treść nie są wysyłane.",
    emptyFolderTitle: "Folder jest pusty",
    emptyFolderText: "W wybranym folderze nie znaleziono widocznych elementów.",
    searching: "Wyszukiwanie...",
    found: (count) => `Znaleziono ${count} wyników`,
    noResultsTitle: "Brak dopasowań",
    noResultsText: "Spróbuj innego słowa lub uwzględnij ukryte pliki.",
    searchFailed: "Wyszukiwanie nie powiodło się",
    cannotComplete: "Nie można ukończyć wyszukiwania",
    openFailed: "Nie udało się otworzyć",
    file: "Plik",
    directory: "Folder",
    expandFolder: "Rozwiń folder",
    collapseFolder: "Zwiń folder",
    unknownTime: "Nieznany czas",
    invalidDirectory: "Wybrany folder nie istnieje lub nie jest folderem.",
    invalidRegex: "Wyrażenie regularne jest nieprawidłowe.",
    searchTaskFailed: "Zadanie wyszukiwania nie powiodło się.",
    openTaskFailed: "Zadanie otwierania nie powiodło się.",
    openPathFailed: "Nie można otworzyć tej ścieżki.",
    updateEyebrow: "Aktualizacje",
    updateTitle: "Aktualizacja",
    checkUpdateButton: "Sprawdź",
    checkingUpdate: "Sprawdzanie aktualizacji...",
    updateIdle: "Sprawdź najnowszą wersję w GitHub Releases.",
    updateAvailable: (version) => `Dostępna jest wersja ${version}.`,
    upToDate: "Używasz najnowszej wersji.",
    currentVersion: "Obecna",
    latestVersion: "Najnowsza",
    publishedAt: "Opublikowano",
    releaseNotes: "Informacje o wydaniu",
    openReleaseButton: "Otwórz wydanie",
    emptyReleaseNotes: "To wydanie nie zawiera notatek.",
    updateCheckFailed: "Nie można sprawdzić aktualizacji.",
    versionCompareFailed: "Nie można porównać wersji.",
    packageChecksums: "Sumy kontrolne pakietów",
    noPackageChecksums: "GitHub nie zwrócił metadanych sum kontrolnych pakietów.",
    updateRateLimited: "Osiągnięto limit dostępu do GitHub. Spróbuj ponownie później.",
    updateNotFound: "Nie znaleziono wydania GitHub.",
    updateNetworkFailed: "Żądanie sieciowe nie powiodło się.",
    cancelSearchButton: "Anuluj",
    statSkipped: "Pominięto",
    resultFilterLabel: "Filtruj wyniki",
    resultFilterPlaceholder: "Filtruj widoczne wyniki",
    typeFilterLabel: "Typ",
    typeAll: "Wszystkie",
    typeFiles: "Pliki",
    typeFolders: "Foldery",
    sortLabel: "Sortuj",
    sortNameAsc: "Nazwa A-Z",
    sortNameDesc: "Nazwa Z-A",
    sortModifiedDesc: "Najnowsze",
    sortModifiedAsc: "Najstarsze",
    sortSizeDesc: "Największe",
    sortSizeAsc: "Najmniejsze",
    progressIdle: "Postęp pojawi się podczas wyszukiwania.",
    scanningProgress: (matches, path) => "Skanowanie... Dopasowań: " + matches + (path ? " · " + path : ""),
    searchCancelled: "Wyszukiwanie anulowane",
    noVisibleResultsTitle: "Brak widocznych wyników",
    noVisibleResultsText: "Dostosuj filtry lub sortowanie.",
    visibleResultCount: (visible, total) => "Wyświetlono " + visible + " z " + total,
    openUpdates: "Sprawdź aktualizacje",
    openSettings: "Otwórz ustawienia",
    closeDialog: "Zamknij okno",
    preferencesEyebrow: "Preferencje",
    settingsTitle: "Ustawienia",
    settingsCategories: "Kategorie ustawień",
    appearanceCategory: "Wygląd",
    languageDescription: "Wybierz język interfejsu.",
    themeTitle: "Motyw",
    themeDescription: "Wybierz gotowy wygląd FileNavigation.",
    lightTheme: "Jasny",
    darkTheme: "Ciemny",
    automaticUpdateButton: "Pobierz, zainstaluj i uruchom ponownie",
    preparingUpdate: "Wykrywanie systemu i przygotowywanie aktualizacji...",
    updateDownloading: (percent) => "Pobieranie aktualizacji... " + percent + "%",
    installingUpdate: "Pobieranie zakończone. Instalowanie...",
    restartingUpdate: "Instalacja zakończona. Ponowne uruchamianie...",
    automaticUpdateFailed: "Automatyczna aktualizacja nie powiodła się.",
    updaterUnavailable: "Brak zgodnego pakietu aktualizacji dla tego systemu.",
    toolsLabel: "Narzędzia", toolSearch: "Wyszukiwanie", toolIntegrity: "Integralność",
    integritySourceLabel: "Źródło", integritySourceFile: "Jeden plik", integritySourceFolder: "Cały folder", integritySourceCurrent: "Bieżący folder wyszukiwania", integritySourceFiltered: "Filtrowane wyniki",
    integrityTargetLabel: "Plik lub folder", integrityTargetPlaceholder: "Wybierz plik lub folder", pickIntegrityTarget: "Wybierz plik lub folder", pickIntegrityFolder: "Wybierz folder do sprawdzenia", pickIntegrityFile: "Wybierz plik do sprawdzenia",
    integrityAlgorithmLabel: "Algorytm", expectedHashLabel: "Oczekiwany odcisk (opcjonalnie)", expectedHashPlaceholder: "Wklej wcześniejszy odcisk do porównania", verifyIntegrityButton: "Sprawdź",
    integrityEyebrow: "Integralność", integrityReady: "Gotowe do sprawdzenia", integrityFiles: "Pliki", integrityBytes: "Odczytano",
    integrityProgressIdle: "Wybierz źródło, aby obliczyć jego odcisk.", integrityEmptyTitle: "Sprawdź pliki lokalnie", integrityEmptyText: "Oblicz odcisk pliku, folderu lub filtrowanych wyników.",
    integrityReading: "Obliczanie odcisków", integrityComplete: "Sprawdzanie zakończone", integrityCompleteDetail: "Odczytano wszystkie pliki i utworzono odcisk zbiorczy.",
    integrityCancelled: "Sprawdzanie anulowane", integrityCancelledDetail: "Operację zatrzymano przed sprawdzeniem wszystkich plików.", integrityFailed: "Sprawdzanie nie powiodło się",
    integrityCompleteTitle: "Odcisk obliczony", integrityCompleteText: "Zapisz odcisk zbiorczy, aby później ponownie sprawdzić ten zestaw.",
    integrityMatchTitle: "Integralność potwierdzona", integrityMatchText: "Obliczony odcisk jest zgodny z oczekiwaną wartością.", integrityMismatchTitle: "Niezgodność integralności", integrityMismatchText: "Zawartość, ścieżki lub zestaw plików różnią się od oczekiwanego odcisku.",
    aggregateFingerprint: "Odcisk zbiorczy", copyFingerprint: "Kopiuj odcisk", fingerprintCopied: "Odcisk skopiowany.", copyFailed: "Nie udało się skopiować odcisku.", fileFingerprints: "Odciski plików",
    invalidFile: "Wybierz prawidłowy plik.", invalidExpectedHash: "Oczekiwany odcisk jest nieprawidłowy dla wybranego algorytmu.", unsupportedHashAlgorithm: "Algorytm skrótu nie jest obsługiwany.",
    integrityTaskFailed: "Nie udało się ukończyć sprawdzania integralności.", fileReadFailed: "Nie udało się odczytać pliku.", fileChangedDuringCheck: "Plik zmienił się podczas sprawdzania. Uruchom ponownie.",
    folderReadFailed: "Nie udało się odczytać całego folderu. Odcisk nie został utworzony.", noFilteredFiles: "Filtrowane wyniki nie zawierają plików.", invalidIntegritySource: "Wybierz prawidłowe źródło."
    ,toolDuplicates: "Duplikaty", duplicatesSourceLabel: "Źródło skanowania", duplicatesSourceFolder: "Wybierz folder", duplicatesSourceCurrent: "Bieżący folder wyszukiwania", duplicatesSourceFiltered: "Filtrowane wyniki", duplicatesFolderLabel: "Folder", duplicatesFolderPlaceholder: "Wybierz folder do skanowania", pickDuplicatesFolder: "Wybierz folder do skanowania", duplicatesSafetyNote: "Pliki są porównywane przez SHA-256. Usunięte kopie trafiają do kosza systemowego.", scanDuplicatesButton: "Skanuj duplikaty",
    duplicatesEyebrow: "Duplikaty plików", duplicatesReady: "Gotowe do skanowania", duplicateGroups: "Grupy", duplicateCopies: "Nadmiarowe kopie", reclaimableSpace: "Do odzyskania", duplicatesProgressIdle: "Wybierz źródło, aby znaleźć identyczne pliki.", duplicatesEmptyTitle: "Bezpiecznie znajdź duplikaty", duplicatesEmptyText: "Grupowane są tylko pliki identyczne bajt po bajcie. Nic nie jest automatycznie zaznaczane ani usuwane.", selectedCopies: "zaznaczono", autoSelectDuplicates: "Zaznacz nadmiarowe kopie", moveSelectedToTrash: "Przenieś do kosza",
    duplicatesConfirmEyebrow: "Potwierdź czyszczenie", duplicatesConfirmTitle: "Przenieść zaznaczone kopie do kosza?", duplicatesConfirmSafety: "W każdej grupie zawsze pozostaje jeden zweryfikowany plik.", cancelAction: "Anuluj", confirmMoveToTrash: "Przenieś do kosza", duplicatesConfirmDetail: (count, size) => `Przenieść ${count} zaznaczonych plików (${size}) do kosza systemowego?`, duplicatesScanning: "Skanowanie duplikatów", duplicatesHashing: "Porównywanie zawartości", duplicatesComplete: "Skanowanie zakończone", duplicatesCompleteDetail: (groups, size) => `Znaleziono grup: ${groups} · do odzyskania ${size}`, duplicatesNoMatches: "Nie znaleziono duplikatów", duplicatesNoMatchesDetail: "W tym źródle nie znaleziono plików identycznych bajt po bajcie.", duplicatesCancelled: "Skanowanie anulowane", duplicatesCancelledDetail: "Skanowanie zatrzymano przed porównaniem wszystkich kandydatów.", duplicatesFailed: "Skanowanie duplikatów nie powiodło się",
    duplicateGroupTitle: (index) => `Grupa ${index}`, keptCopy: "Zachowaj", openItem: "Otwórz plik", duplicateKeepRequired: "W każdej grupie musi pozostać co najmniej jeden plik.", duplicateTaskFailed: "Nie udało się zakończyć skanowania.", invalidDuplicateSource: "Wybierz prawidłowe źródło.", duplicateCancelled: "Skanowanie duplikatów anulowano.", duplicateSelectionChanged: "Wybrane pliki zmieniły się lub nie są już identyczne. Zeskanuj ponownie.", noDuplicateSelection: "Wybierz co najmniej jedną kopię.", duplicateDeleteFailed: "Nie udało się przenieść plików do kosza.", duplicatesDeleteComplete: "Czyszczenie zakończone", duplicatesDeleteCompleteDetail: "Wybrane kopie przeniesiono do kosza.", duplicatesDeletedDetail: (count, size) => `Przeniesiono ${count} plików · odzyskano ${size}`, duplicatesDeletePartial: "Czyszczenie częściowe", duplicatesDeletePartialDetail: (deleted, failed) => `Przeniesiono ${deleted} plików; ${failed} nie udało się przenieść.`
};

export default messages;
