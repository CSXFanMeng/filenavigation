const messages = {
    brandSubtitle: "Lokale Dateisuche",
    minimizeWindow: "Fenster minimieren",
    maximizeWindow: "Fenster maximieren oder wiederherstellen",
    closeWindow: "Fenster schließen",
    languageLabel: "Sprache",
    rootLabel: "Suchordner",
    rootPlaceholder: "Ordnerpfad wählen oder eingeben",
    pickDirectory: "Ordner wählen",
    queryLabel: "Dateiname-Schlüsselwort",
    queryPlaceholder: "Zum Beispiel report, .rs, Rechnung",
    regexPlaceholder: "Zum Beispiel ^report_\\d{4}\\.pdf$",
    caseSensitive: "Groß-/Kleinschreibung",
    useRegex: "Regulärer Ausdruck",
    includeHidden: "Versteckte Dateien einbeziehen",
    searchButton: "Suchen",
    eyebrow: "Suche",
    waiting: "Bereit zur Suche",
    statFiles: "Dateien geprüft",
    statDirs: "Ordner geprüft",
    statTime: "Zeit",
    emptyTitle: "Wählen Sie einen Ordner zum Durchsuchen oder geben Sie ein Stichwort für den Dateinamen ein",
    emptyText: "Die Suche läuft lokal. Dateinamen, Pfade und Inhalte werden nicht hochgeladen.",
    emptyFolderTitle: "Der Ordner ist leer",
    emptyFolderText: "Im ausgewählten Ordner wurden keine sichtbaren Einträge gefunden.",
    searching: "Suche...",
    found: (count) => `${count} Treffer gefunden`,
    noResultsTitle: "Keine Treffer",
    noResultsText: "Versuchen Sie ein anderes Schlüsselwort oder versteckte Dateien.",
    searchFailed: "Suche fehlgeschlagen",
    cannotComplete: "Die Suche konnte nicht abgeschlossen werden",
    openFailed: "Öffnen fehlgeschlagen",
    file: "Datei",
    directory: "Ordner",
    expandFolder: "Ordner erweitern",
    collapseFolder: "Ordner reduzieren",
    unknownTime: "Unbekannte Zeit",
    invalidDirectory: "Der ausgewählte Ordner existiert nicht oder ist kein Ordner.",
    invalidRegex: "Der reguläre Ausdruck ist ungültig.",
    searchTaskFailed: "Die Suchaufgabe ist fehlgeschlagen.",
    openTaskFailed: "Die Öffnungsaufgabe ist fehlgeschlagen.",
    openPathFailed: "Dieser Pfad konnte nicht geöffnet werden.",
    updateEyebrow: "Updates",
    updateTitle: "Softwareupdate",
    checkUpdateButton: "Prüfen",
    checkingUpdate: "Suche nach Updates...",
    updateIdle: "GitHub Releases nach der neuesten Version prüfen.",
    updateAvailable: (version) => `Version ${version} ist verfügbar.`,
    upToDate: "Sie verwenden die neueste Version.",
    currentVersion: "Aktuell",
    latestVersion: "Neueste",
    publishedAt: "Veröffentlicht",
    releaseNotes: "Versionshinweise",
    openReleaseButton: "Release öffnen",
    emptyReleaseNotes: "Diese Version enthält keine Hinweise.",
    updateCheckFailed: "Updates konnten nicht geprüft werden.",
    versionCompareFailed: "Versionen konnten nicht verglichen werden.",
    packageChecksums: "Paketprüfsummen",
    noPackageChecksums: "GitHub hat keine Prüfsummen-Metadaten für Pakete zurückgegeben.",
    updateRateLimited: "Das GitHub-Zugriffslimit wurde erreicht. Bitte später erneut versuchen.",
    updateNotFound: "Keine GitHub-Veröffentlichung gefunden.",
    updateNetworkFailed: "Die Netzwerkanfrage ist fehlgeschlagen.",
    cancelSearchButton: "Abbrechen",
    statSkipped: "Übersprungen",
    resultFilterLabel: "Ergebnisse filtern",
    resultFilterPlaceholder: "Sichtbare Ergebnisse filtern",
    typeFilterLabel: "Typ",
    typeAll: "Alle",
    typeFiles: "Dateien",
    typeFolders: "Ordner",
    sortLabel: "Sortieren",
    sortNameAsc: "Name A-Z",
    sortNameDesc: "Name Z-A",
    sortModifiedDesc: "Neueste",
    sortModifiedAsc: "Älteste",
    sortSizeDesc: "Größte",
    sortSizeAsc: "Kleinste",
    progressIdle: "Der Fortschritt wird während der Suche angezeigt.",
    scanningProgress: (matches, path) => "Durchsuche... " + matches + " Treffer" + (path ? " · " + path : ""),
    searchCancelled: "Suche abgebrochen",
    noVisibleResultsTitle: "Keine sichtbaren Ergebnisse",
    noVisibleResultsText: "Filter oder Sortierung anpassen.",
    visibleResultCount: (visible, total) => visible + " von " + total + " Ergebnissen",
    openUpdates: "Nach Updates suchen",
    openSettings: "Einstellungen öffnen",
    closeDialog: "Fenster schließen",
    preferencesEyebrow: "Voreinstellungen",
    settingsTitle: "Einstellungen",
    settingsCategories: "Einstellungskategorien",
    appearanceCategory: "Darstellung",
    languageDescription: "Wähle die Sprache der Benutzeroberfläche.",
    themeTitle: "Design",
    themeDescription: "Wählen Sie ein voreingestelltes Erscheinungsbild für FileNavigation.",
    lightTheme: "Hell",
    darkTheme: "Dunkel",
    automaticUpdateButton: "Herunterladen, installieren und neu starten",
    preparingUpdate: "System wird erkannt und das Update vorbereitet...",
    updateDownloading: (percent) => "Update wird heruntergeladen... " + percent + "%",
    installingUpdate: "Download abgeschlossen. Installation läuft...",
    restartingUpdate: "Installation abgeschlossen. Neustart...",
    automaticUpdateFailed: "Automatisches Update fehlgeschlagen.",
    updaterUnavailable: "Für dieses System ist kein kompatibles Update-Paket verfügbar.",
    toolsLabel: "Werkzeuge", toolSearch: "Suche", toolIntegrity: "Integrität",
    integritySourceLabel: "Quelle", integritySourceFile: "Einzelne Datei", integritySourceFolder: "Ganzer Ordner", integritySourceCurrent: "Aktueller Suchordner", integritySourceFiltered: "Gefilterte Ergebnisse",
    integrityTargetLabel: "Datei oder Ordner", integrityTargetPlaceholder: "Datei oder Ordner wählen", pickIntegrityTarget: "Datei oder Ordner wählen", pickIntegrityFolder: "Zu prüfenden Ordner wählen", pickIntegrityFile: "Zu prüfende Datei wählen",
    integrityAlgorithmLabel: "Algorithmus", expectedHashLabel: "Erwarteter Fingerabdruck (optional)", expectedHashPlaceholder: "Früheren Fingerabdruck zum Vergleich einfügen", verifyIntegrityButton: "Prüfen",
    integrityEyebrow: "Integrität", integrityReady: "Bereit zur Prüfung", integrityFiles: "Dateien", integrityBytes: "Gelesen",
    integrityProgressIdle: "Quelle wählen, um ihren Fingerabdruck zu berechnen.", integrityEmptyTitle: "Dateien lokal prüfen", integrityEmptyText: "Fingerabdruck einer Datei, eines Ordners oder der gefilterten Ergebnisse berechnen.",
    integrityReading: "Fingerabdrücke werden berechnet", integrityComplete: "Prüfung abgeschlossen", integrityCompleteDetail: "Alle Dateien wurden gelesen und der Gesamtfingerabdruck ist bereit.",
    integrityCancelled: "Prüfung abgebrochen", integrityCancelledDetail: "Der Vorgang wurde vor Abschluss aller Dateien beendet.", integrityFailed: "Prüfung fehlgeschlagen",
    integrityCompleteTitle: "Fingerabdruck berechnet", integrityCompleteText: "Gesamtfingerabdruck speichern, um diese Dateigruppe später erneut zu prüfen.",
    integrityMatchTitle: "Integrität bestätigt", integrityMatchText: "Der berechnete Fingerabdruck entspricht dem erwarteten Wert.",
    integrityMismatchTitle: "Integrität stimmt nicht überein", integrityMismatchText: "Inhalt, Pfade oder Dateigruppe unterscheiden sich vom erwarteten Fingerabdruck.",
    aggregateFingerprint: "Gesamtfingerabdruck", copyFingerprint: "Fingerabdruck kopieren", fingerprintCopied: "Fingerabdruck kopiert.", copyFailed: "Fingerabdruck konnte nicht kopiert werden.", fileFingerprints: "Datei-Fingerabdrücke",
    invalidFile: "Gültige Datei wählen.", invalidExpectedHash: "Der erwartete Fingerabdruck passt nicht zum gewählten Algorithmus.", unsupportedHashAlgorithm: "Der Hash-Algorithmus wird nicht unterstützt.",
    integrityTaskFailed: "Die Integritätsprüfung konnte nicht abgeschlossen werden.", fileReadFailed: "Eine Datei konnte nicht gelesen werden.", fileChangedDuringCheck: "Eine Datei wurde während der Prüfung geändert. Erneut prüfen.",
    folderReadFailed: "Der Ordner konnte nicht vollständig gelesen werden. Es wurde kein Fingerabdruck erstellt.", noFilteredFiles: "Die gefilterten Ergebnisse enthalten keine Dateien.", invalidIntegritySource: "Gültige Quelle wählen."
    ,toolDuplicates: "Duplikate", duplicatesSourceLabel: "Scanquelle", duplicatesSourceFolder: "Ordner wählen", duplicatesSourceCurrent: "Aktueller Suchordner", duplicatesSourceFiltered: "Gefilterte Ergebnisse",
    duplicatesFolderLabel: "Ordner", duplicatesFolderPlaceholder: "Zu scannenden Ordner wählen", pickDuplicatesFolder: "Ordner zum Scannen wählen", duplicatesSafetyNote: "Dateien werden per SHA-256 verglichen. Entfernte Kopien landen im Systempapierkorb.", scanDuplicatesButton: "Duplikate suchen",
    duplicatesEyebrow: "Doppelte Dateien", duplicatesReady: "Bereit zum Scannen", duplicateGroups: "Gruppen", duplicateCopies: "Zusätzliche Kopien", reclaimableSpace: "Freigebbar", duplicatesProgressIdle: "Quelle wählen, um identische Dateien zu finden.",
    duplicatesEmptyTitle: "Duplikate sicher finden", duplicatesEmptyText: "Nur bytegleiche Dateien werden gruppiert. Nichts wird automatisch ausgewählt oder entfernt.", selectedCopies: "ausgewählt", autoSelectDuplicates: "Zusätzliche Kopien wählen", moveSelectedToTrash: "In Papierkorb",
    duplicatesConfirmEyebrow: "Bereinigung bestätigen", duplicatesConfirmTitle: "Ausgewählte Kopien in den Papierkorb verschieben?", duplicatesConfirmSafety: "In jeder Gruppe bleibt immer eine geprüfte Datei erhalten.", cancelAction: "Abbrechen", confirmMoveToTrash: "In Papierkorb", duplicatesConfirmDetail: (count, size) => `${count} ausgewählte Dateien (${size}) in den Systempapierkorb verschieben?`,
    duplicatesScanning: "Duplikate werden gesucht", duplicatesHashing: "Dateiinhalte werden verglichen", duplicatesComplete: "Duplikatsuche abgeschlossen", duplicatesCompleteDetail: (groups, size) => `${groups} Gruppen gefunden · ${size} freigebbar`, duplicatesNoMatches: "Keine Duplikate gefunden", duplicatesNoMatchesDetail: "In dieser Quelle wurden keine bytegleichen Dateien gefunden.", duplicatesCancelled: "Duplikatsuche abgebrochen", duplicatesCancelledDetail: "Der Scan wurde vor dem Vergleich aller Kandidaten beendet.", duplicatesFailed: "Duplikatsuche fehlgeschlagen",
    duplicateGroupTitle: (index) => `Gruppe ${index}`, keptCopy: "Behalten", openItem: "Datei öffnen", duplicateKeepRequired: "In jeder Gruppe muss mindestens eine Datei verbleiben.", duplicateTaskFailed: "Die Duplikatsuche konnte nicht abgeschlossen werden.", invalidDuplicateSource: "Gültige Scanquelle wählen.", duplicateCancelled: "Die Duplikatsuche wurde abgebrochen.", duplicateSelectionChanged: "Die gewählten Dateien wurden geändert oder sind nicht mehr identisch. Erneut scannen.", noDuplicateSelection: "Mindestens eine Kopie auswählen.", duplicateDeleteFailed: "Die Dateien konnten nicht in den Papierkorb verschoben werden.",
    duplicatesDeleteComplete: "Bereinigung abgeschlossen", duplicatesDeleteCompleteDetail: "Die ausgewählten Kopien wurden in den Papierkorb verschoben.", duplicatesDeletedDetail: (count, size) => `${count} Dateien verschoben · ${size} freigegeben`, duplicatesDeletePartial: "Bereinigung teilweise abgeschlossen", duplicatesDeletePartialDetail: (deleted, failed) => `${deleted} Dateien verschoben; ${failed} konnten nicht verschoben werden.`
};

export default messages;
