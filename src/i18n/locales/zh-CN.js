const messages = {
    brandSubtitle: "本地目录文件查找",
    minimizeWindow: "最小化窗口",
    maximizeWindow: "最大化或还原窗口",
    closeWindow: "关闭窗口",
    languageLabel: "语言",
    rootLabel: "搜索目录",
    rootPlaceholder: "选择或输入目录路径",
    pickDirectory: "选择目录",
    queryLabel: "文件名关键字",
    queryPlaceholder: "例如 report、.rs、合同",
    regexPlaceholder: "例如 ^报告_\\d{4}\\.pdf$",
    caseSensitive: "区分大小写",
    useRegex: "正则表达式",
    includeHidden: "包含隐藏文件",
    searchButton: "开始查找",
    eyebrow: "搜索",
    waiting: "等待搜索",
    statFiles: "扫描文件",
    statDirs: "扫描目录",
    statTime: "耗时",
    emptyTitle: "选择目录即可浏览，也可输入文件名关键字查找",
    emptyText: "搜索在本机完成，不上传文件名、路径或内容。",
    emptyFolderTitle: "目录为空",
    emptyFolderText: "所选目录中没有可显示的内容。",
    searching: "正在查找...",
    found: (count) => `找到 ${count} 个结果`,
    noResultsTitle: "没有匹配结果",
    noResultsText: "换一个关键字，或启用隐藏文件搜索。",
    searchFailed: "搜索失败",
    cannotComplete: "无法完成搜索",
    openFailed: "打开失败",
    file: "文件",
    directory: "目录",
    expandFolder: "展开目录",
    collapseFolder: "折叠目录",
    unknownTime: "未知时间",
    invalidDirectory: "搜索目录不存在或不是目录。",
    invalidRegex: "正则表达式无效。",
    searchTaskFailed: "搜索任务失败。",
    openTaskFailed: "打开任务失败。",
    openPathFailed: "无法打开路径。",
    updateEyebrow: "更新",
    updateTitle: "软件更新",
    checkUpdateButton: "检查",
    checkingUpdate: "正在检查更新...",
    updateIdle: "检查 GitHub Releases 中的最新版本。",
    updateAvailable: (version) => `发现新版本 ${version}。`,
    upToDate: "当前已是最新版本。",
    currentVersion: "当前版本",
    latestVersion: "最新版本",
    publishedAt: "发布时间",
    releaseNotes: "更新日志",
    openReleaseButton: "打开发布页",
    emptyReleaseNotes: "此版本没有更新日志。",
    updateCheckFailed: "无法检查更新。",
    versionCompareFailed: "无法比较版本。",
    packageChecksums: "安装包校验值",
    noPackageChecksums: "GitHub 未返回安装包校验元数据。",
    updateRateLimited: "GitHub 访问频率已达上限，请稍后再试。",
    updateNotFound: "没有找到 GitHub Release。",
    updateNetworkFailed: "网络请求失败。",
    cancelSearchButton: "取消",
    statSkipped: "跳过",
    resultFilterLabel: "结果过滤",
    resultFilterPlaceholder: "过滤当前结果",
    typeFilterLabel: "类型",
    typeAll: "全部",
    typeFiles: "文件",
    typeFolders: "目录",
    sortLabel: "排序",
    sortNameAsc: "名称 A-Z",
    sortNameDesc: "名称 Z-A",
    sortModifiedDesc: "最新修改",
    sortModifiedAsc: "最早修改",
    sortSizeDesc: "最大文件",
    sortSizeAsc: "最小文件",
    progressIdle: "搜索时会显示进度详情。",
    scanningProgress: (matches, path) => `正在扫描... ${matches} 个匹配${path ? ` · ${path}` : ""}`,
    searchCancelled: "搜索已取消",
    noVisibleResultsTitle: "没有可见结果",
    noVisibleResultsText: "调整结果过滤或排序条件。",
    visibleResultCount: (visible, total) => `显示 ${visible} / ${total} 个结果`,
    openUpdates: "检查更新",
    openSettings: "打开设置",
    closeDialog: "关闭窗口",
    preferencesEyebrow: "偏好设置",
    settingsTitle: "设置",
    settingsCategories: "设置分类",
    appearanceCategory: "外观",
    languageDescription: "选择界面显示语言。",
    themeTitle: "主题",
    themeDescription: "选择 FileNavigation 的预设外观。",
    lightTheme: "浅色",
    darkTheme: "深色",
    automaticUpdateButton: "下载、安装并重启",
    preparingUpdate: "正在识别系统并准备更新...",
    updateDownloading: (percent) => "正在下载更新... " + percent + "%",
    installingUpdate: "下载完成，正在安装...",
    restartingUpdate: "安装完成，正在重启...",
    automaticUpdateFailed: "自动更新失败。",
    updaterUnavailable: "没有适用于当前系统的更新包。",
    toolsLabel: "工具",
    toolSearch: "文件搜索",
    toolIntegrity: "完整性校验",
    integritySourceLabel: "校验来源",
    integritySourceFile: "单个文件",
    integritySourceFolder: "整个文件夹",
    integritySourceCurrent: "当前搜索目录",
    integritySourceFiltered: "当前筛选结果",
    integrityTargetLabel: "文件或文件夹",
    integrityTargetPlaceholder: "选择文件或文件夹",
    pickIntegrityTarget: "选择文件或文件夹",
    pickIntegrityFolder: "选择要校验的文件夹",
    pickIntegrityFile: "选择要校验的文件",
    integrityAlgorithmLabel: "哈希算法",
    expectedHashLabel: "预期指纹（可选）",
    expectedHashPlaceholder: "粘贴之前保存的指纹以进行比对",
    verifyIntegrityButton: "开始校验",
    integrityEyebrow: "完整性",
    integrityReady: "等待校验",
    integrityFiles: "文件",
    integrityBytes: "已读取",
    integrityProgressIdle: "选择来源以计算文件指纹。",
    integrityEmptyTitle: "在本地校验文件",
    integrityEmptyText: "可计算单个文件、整个文件夹或当前筛选结果的指纹。",
    integrityReading: "正在计算文件指纹",
    integrityComplete: "校验完成",
    integrityCompleteDetail: "所有文件均已读取，整体指纹已生成。",
    integrityCancelled: "校验已取消",
    integrityCancelledDetail: "操作已在全部文件校验完成前停止。",
    integrityFailed: "校验失败",
    integrityCompleteTitle: "指纹计算完成",
    integrityCompleteText: "保存整体指纹，之后可再次验证这组文件。",
    integrityMatchTitle: "完整性验证通过",
    integrityMatchText: "计算出的指纹与预期值一致。",
    integrityMismatchTitle: "完整性不一致",
    integrityMismatchText: "文件内容、相对路径或文件集合与预期指纹不一致。",
    aggregateFingerprint: "整体指纹",
    copyFingerprint: "复制指纹",
    fingerprintCopied: "指纹已复制。",
    copyFailed: "无法复制指纹。",
    fileFingerprints: "各文件指纹",
    invalidFile: "请选择有效的文件。",
    invalidExpectedHash: "预期指纹不符合所选算法的格式。",
    unsupportedHashAlgorithm: "不支持所选的哈希算法。",
    integrityTaskFailed: "无法完成完整性校验任务。",
    fileReadFailed: "无法读取某个文件。",
    fileChangedDuringCheck: "校验过程中有文件发生变化，请重新校验。",
    folderReadFailed: "无法完整读取文件夹，因此未生成指纹。",
    noFilteredFiles: "当前筛选结果中没有文件。",
    invalidIntegritySource: "请选择有效的校验来源。"
};

export default messages;
