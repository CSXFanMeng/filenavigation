const messages = {
    brandSubtitle: "Локальный поиск файлов",
    minimizeWindow: "Свернуть окно",
    maximizeWindow: "Развернуть или восстановить окно",
    closeWindow: "Закрыть окно",
    languageLabel: "Язык",
    rootLabel: "Папка поиска",
    rootPlaceholder: "Выберите или введите путь к папке",
    pickDirectory: "Выбрать папку",
    queryLabel: "Ключевое слово имени файла",
    queryPlaceholder: "Например report, .rs, договор",
    regexPlaceholder: "Например ^report_\\d{4}\\.pdf$",
    caseSensitive: "Учитывать регистр",
    useRegex: "Регулярное выражение",
    includeHidden: "Включать скрытые файлы",
    searchButton: "Искать",
    eyebrow: "Поиск",
    waiting: "Готово к поиску",
    statFiles: "Файлов проверено",
    statDirs: "Папок проверено",
    statTime: "Время",
    emptyTitle: "Выберите папку для просмотра или введите ключевое слово имени файла",
    emptyText: "Поиск выполняется локально. Имена, пути и содержимое не отправляются.",
    emptyFolderTitle: "Папка пуста",
    emptyFolderText: "В выбранной папке нет видимых элементов.",
    searching: "Поиск...",
    found: (count) => `Найдено результатов: ${count}`,
    noResultsTitle: "Совпадений нет",
    noResultsText: "Попробуйте другое ключевое слово или включите скрытые файлы.",
    searchFailed: "Ошибка поиска",
    cannotComplete: "Не удалось завершить поиск",
    openFailed: "Не удалось открыть",
    file: "Файл",
    directory: "Папка",
    expandFolder: "Развернуть папку",
    collapseFolder: "Свернуть папку",
    unknownTime: "Неизвестное время",
    invalidDirectory: "Выбранная папка не существует или не является папкой.",
    invalidRegex: "Регулярное выражение недействительно.",
    searchTaskFailed: "Задача поиска завершилась ошибкой.",
    openTaskFailed: "Задача открытия завершилась ошибкой.",
    openPathFailed: "Не удалось открыть этот путь.",
    updateEyebrow: "Обновления",
    updateTitle: "Обновление ПО",
    checkUpdateButton: "Проверить",
    checkingUpdate: "Проверка обновлений...",
    updateIdle: "Проверить последнюю версию в GitHub Releases.",
    updateAvailable: (version) => `Доступна версия ${version}.`,
    upToDate: "Установлена последняя версия.",
    currentVersion: "Текущая",
    latestVersion: "Последняя",
    publishedAt: "Опубликовано",
    releaseNotes: "Журнал изменений",
    openReleaseButton: "Открыть релиз",
    emptyReleaseNotes: "У этого релиза нет описания.",
    updateCheckFailed: "Не удалось проверить обновления.",
    versionCompareFailed: "Не удалось сравнить версии.",
    packageChecksums: "Контрольные суммы пакетов",
    noPackageChecksums: "GitHub не вернул данные контрольных сумм пакетов.",
    updateRateLimited: "Достигнут лимит обращений к GitHub. Повторите попытку позже.",
    updateNotFound: "Выпуск GitHub не найден.",
    updateNetworkFailed: "Сетевой запрос завершился ошибкой.",
    cancelSearchButton: "Отмена",
    statSkipped: "Пропущено",
    resultFilterLabel: "Фильтр результатов",
    resultFilterPlaceholder: "Фильтровать видимые результаты",
    typeFilterLabel: "Тип",
    typeAll: "Все",
    typeFiles: "Файлы",
    typeFolders: "Папки",
    sortLabel: "Сортировка",
    sortNameAsc: "Имя А-Я",
    sortNameDesc: "Имя Я-А",
    sortModifiedDesc: "Сначала новые",
    sortModifiedAsc: "Сначала старые",
    sortSizeDesc: "Сначала крупные",
    sortSizeAsc: "Сначала малые",
    progressIdle: "Ход выполнения появится во время поиска.",
    scanningProgress: (matches, path) => "Сканирование... Совпадений: " + matches + (path ? " · " + path : ""),
    searchCancelled: "Поиск отменён",
    noVisibleResultsTitle: "Нет видимых результатов",
    noVisibleResultsText: "Измените фильтры или сортировку.",
    visibleResultCount: (visible, total) => "Показано " + visible + " из " + total,
    openUpdates: "Проверить обновления",
    openSettings: "Открыть настройки",
    closeDialog: "Закрыть окно",
    preferencesEyebrow: "Параметры",
    settingsTitle: "Настройки",
    settingsCategories: "Категории настроек",
    appearanceCategory: "Оформление",
    languageDescription: "Выберите язык интерфейса.",
    themeTitle: "Тема",
    themeDescription: "Выберите готовое оформление FileNavigation.",
    lightTheme: "Светлая",
    darkTheme: "Тёмная",
    automaticUpdateButton: "Скачать, установить и перезапустить",
    preparingUpdate: "Определение системы и подготовка обновления...",
    updateDownloading: (percent) => "Загрузка обновления... " + percent + "%",
    installingUpdate: "Загрузка завершена. Установка...",
    restartingUpdate: "Установка завершена. Перезапуск...",
    automaticUpdateFailed: "Не удалось выполнить автоматическое обновление.",
    updaterUnavailable: "Для этой системы нет совместимого пакета обновления.",
    toolsLabel: "Инструменты", toolSearch: "Поиск", toolIntegrity: "Целостность",
    integritySourceLabel: "Источник", integritySourceFile: "Один файл", integritySourceFolder: "Вся папка", integritySourceCurrent: "Текущая папка поиска", integritySourceFiltered: "Отфильтрованные результаты",
    integrityTargetLabel: "Файл или папка", integrityTargetPlaceholder: "Выберите файл или папку", pickIntegrityTarget: "Выбрать файл или папку", pickIntegrityFolder: "Выбрать папку для проверки", pickIntegrityFile: "Выбрать файл для проверки",
    integrityAlgorithmLabel: "Алгоритм", expectedHashLabel: "Ожидаемый отпечаток (необязательно)", expectedHashPlaceholder: "Вставьте прежний отпечаток для сравнения", verifyIntegrityButton: "Проверить",
    integrityEyebrow: "Целостность", integrityReady: "Готово к проверке", integrityFiles: "Файлы", integrityBytes: "Прочитано",
    integrityProgressIdle: "Выберите источник для вычисления отпечатка.", integrityEmptyTitle: "Локальная проверка файлов", integrityEmptyText: "Вычислите отпечаток файла, папки или отфильтрованных результатов.",
    integrityReading: "Вычисление отпечатков", integrityComplete: "Проверка завершена", integrityCompleteDetail: "Все файлы прочитаны, общий отпечаток готов.",
    integrityCancelled: "Проверка отменена", integrityCancelledDetail: "Операция остановлена до проверки всех файлов.", integrityFailed: "Ошибка проверки",
    integrityCompleteTitle: "Отпечаток вычислен", integrityCompleteText: "Сохраните общий отпечаток для повторной проверки набора.",
    integrityMatchTitle: "Целостность подтверждена", integrityMatchText: "Вычисленный отпечаток совпадает с ожидаемым.", integrityMismatchTitle: "Несоответствие целостности", integrityMismatchText: "Содержимое, пути или набор файлов отличаются от ожидаемого.",
    aggregateFingerprint: "Общий отпечаток", copyFingerprint: "Копировать отпечаток", fingerprintCopied: "Отпечаток скопирован.", copyFailed: "Не удалось скопировать отпечаток.", fileFingerprints: "Отпечатки файлов",
    invalidFile: "Выберите допустимый файл.", invalidExpectedHash: "Ожидаемый отпечаток неверен для выбранного алгоритма.", unsupportedHashAlgorithm: "Алгоритм хеширования не поддерживается.",
    integrityTaskFailed: "Не удалось завершить проверку целостности.", fileReadFailed: "Не удалось прочитать файл.", fileChangedDuringCheck: "Файл изменился во время проверки. Запустите её снова.",
    folderReadFailed: "Не удалось полностью прочитать папку. Отпечаток не создан.", noFilteredFiles: "В отфильтрованных результатах нет файлов.", invalidIntegritySource: "Выберите допустимый источник."
    ,toolDuplicates: "Дубликаты", duplicatesSourceLabel: "Источник", duplicatesSourceFolder: "Выбрать папку", duplicatesSourceCurrent: "Текущая папка поиска", duplicatesSourceFiltered: "Отфильтрованные результаты", duplicatesFolderLabel: "Папка", duplicatesFolderPlaceholder: "Выберите папку для сканирования", pickDuplicatesFolder: "Выбрать папку", duplicatesSafetyNote: "Файлы сравниваются по SHA-256. Удалённые копии перемещаются в системную корзину.", scanDuplicatesButton: "Найти дубликаты",
    duplicatesEyebrow: "Дубликаты файлов", duplicatesReady: "Готово к сканированию", duplicateGroups: "Группы", duplicateCopies: "Лишние копии", reclaimableSpace: "Можно освободить", duplicatesProgressIdle: "Выберите источник для поиска одинаковых файлов.", duplicatesEmptyTitle: "Безопасный поиск дубликатов", duplicatesEmptyText: "Группируются только побайтово одинаковые файлы. Ничего не выбирается и не удаляется автоматически.", selectedCopies: "выбрано", autoSelectDuplicates: "Выбрать лишние копии", moveSelectedToTrash: "В корзину",
    duplicatesConfirmEyebrow: "Подтверждение очистки", duplicatesConfirmTitle: "Переместить выбранные копии в корзину?", duplicatesConfirmSafety: "В каждой группе всегда сохраняется один проверенный файл.", cancelAction: "Отмена", confirmMoveToTrash: "В корзину", duplicatesConfirmDetail: (count, size) => `Переместить ${count} выбранных файлов (${size}) в системную корзину?`, duplicatesScanning: "Поиск дубликатов", duplicatesHashing: "Сравнение содержимого", duplicatesComplete: "Сканирование завершено", duplicatesCompleteDetail: (groups, size) => `Найдено групп: ${groups} · можно освободить ${size}`, duplicatesNoMatches: "Дубликаты не найдены", duplicatesNoMatchesDetail: "В этом источнике нет побайтово одинаковых файлов.", duplicatesCancelled: "Сканирование отменено", duplicatesCancelledDetail: "Сканирование остановлено до сравнения всех кандидатов.", duplicatesFailed: "Ошибка поиска дубликатов",
    duplicateGroupTitle: (index) => `Группа ${index}`, keptCopy: "Оставить", openItem: "Открыть файл", duplicateKeepRequired: "В каждой группе должен остаться хотя бы один файл.", duplicateTaskFailed: "Не удалось завершить поиск дубликатов.", invalidDuplicateSource: "Выберите допустимый источник.", duplicateCancelled: "Поиск дубликатов отменён.", duplicateSelectionChanged: "Выбранные файлы изменились или больше не совпадают. Выполните сканирование снова.", noDuplicateSelection: "Выберите хотя бы одну копию.", duplicateDeleteFailed: "Не удалось переместить файлы в корзину.", duplicatesDeleteComplete: "Очистка завершена", duplicatesDeleteCompleteDetail: "Выбранные копии перемещены в корзину.", duplicatesDeletedDetail: (count, size) => `Перемещено файлов: ${count} · освобождено ${size}`, duplicatesDeletePartial: "Очистка выполнена частично", duplicatesDeletePartialDetail: (deleted, failed) => `Перемещено: ${deleted}; не удалось переместить: ${failed}.`
};

export default messages;
