const messages = {
    brandSubtitle: "Búsqueda local de archivos",
    minimizeWindow: "Minimizar ventana",
    maximizeWindow: "Maximizar o restaurar ventana",
    closeWindow: "Cerrar ventana",
    languageLabel: "Idioma",
    rootLabel: "Carpeta de búsqueda",
    rootPlaceholder: "Elige o escribe una ruta de carpeta",
    pickDirectory: "Elegir carpeta",
    queryLabel: "Palabra clave del archivo",
    queryPlaceholder: "Por ejemplo report, .rs, factura",
    regexPlaceholder: "Por ejemplo ^report_\\d{4}\\.pdf$",
    caseSensitive: "Distinguir mayúsculas",
    useRegex: "Expresión regular",
    includeHidden: "Incluir archivos ocultos",
    searchButton: "Buscar",
    eyebrow: "Buscar",
    waiting: "Listo para buscar",
    statFiles: "Archivos escaneados",
    statDirs: "Carpetas escaneadas",
    statTime: "Tiempo",
    emptyTitle: "Selecciona una carpeta para explorar o escribe una palabra clave de nombre de archivo",
    emptyText: "La búsqueda se ejecuta localmente. No se suben nombres, rutas ni contenido.",
    emptyFolderTitle: "La carpeta está vacía",
    emptyFolderText: "No se encontraron elementos visibles en la carpeta seleccionada.",
    searching: "Buscando...",
    found: (count) => `${count} resultado${count === 1 ? "" : "s"} encontrado${count === 1 ? "" : "s"}`,
    noResultsTitle: "Sin coincidencias",
    noResultsText: "Prueba otra palabra clave o incluye archivos ocultos.",
    searchFailed: "Error de búsqueda",
    cannotComplete: "No se pudo completar la búsqueda",
    openFailed: "No se pudo abrir",
    file: "Archivo",
    directory: "Carpeta",
    expandFolder: "Expandir carpeta",
    collapseFolder: "Contraer carpeta",
    unknownTime: "Hora desconocida",
    invalidDirectory: "La carpeta seleccionada no existe o no es una carpeta.",
    invalidRegex: "La expresión regular no es válida.",
    searchTaskFailed: "La tarea de búsqueda falló.",
    openTaskFailed: "La tarea de apertura falló.",
    openPathFailed: "No se pudo abrir esta ruta.",
    updateEyebrow: "Actualizaciones",
    updateTitle: "Actualización",
    checkUpdateButton: "Comprobar",
    checkingUpdate: "Buscando actualizaciones...",
    updateIdle: "Consulta GitHub Releases para ver la versión más reciente.",
    updateAvailable: (version) => `La versión ${version} está disponible.`,
    upToDate: "Ya estás usando la versión más reciente.",
    currentVersion: "Actual",
    latestVersion: "Más reciente",
    publishedAt: "Publicado",
    releaseNotes: "Notas de la versión",
    openReleaseButton: "Abrir release",
    emptyReleaseNotes: "Esta versión no incluye notas.",
    updateCheckFailed: "No se pudo comprobar actualizaciones.",
    versionCompareFailed: "No se pudieron comparar las versiones.",
    packageChecksums: "Sumas de comprobación de paquetes",
    noPackageChecksums: "GitHub no devolvió metadatos de comprobación del paquete.",
    updateRateLimited: "Se alcanzó el límite de acceso a GitHub. Inténtalo más tarde.",
    updateNotFound: "No se encontró ninguna release de GitHub.",
    updateNetworkFailed: "La solicitud de red falló.",
    cancelSearchButton: "Cancelar",
    statSkipped: "Omitidos",
    resultFilterLabel: "Filtrar resultados",
    resultFilterPlaceholder: "Filtrar resultados visibles",
    typeFilterLabel: "Tipo",
    typeAll: "Todos",
    typeFiles: "Archivos",
    typeFolders: "Carpetas",
    sortLabel: "Ordenar",
    sortNameAsc: "Nombre A-Z",
    sortNameDesc: "Nombre Z-A",
    sortModifiedDesc: "Más recientes",
    sortModifiedAsc: "Más antiguos",
    sortSizeDesc: "Más grandes",
    sortSizeAsc: "Más pequeños",
    progressIdle: "El progreso aparecerá durante la búsqueda.",
    scanningProgress: (matches, path) => "Escaneando... " + matches + " coincidencias" + (path ? " · " + path : ""),
    searchCancelled: "Búsqueda cancelada",
    noVisibleResultsTitle: "No hay resultados visibles",
    noVisibleResultsText: "Ajusta los filtros o el orden.",
    visibleResultCount: (visible, total) => "Mostrando " + visible + " de " + total + " resultados",
    openUpdates: "Buscar actualizaciones",
    openSettings: "Abrir ajustes",
    closeDialog: "Cerrar ventana",
    preferencesEyebrow: "Preferencias",
    settingsTitle: "Ajustes",
    settingsCategories: "Categorías de ajustes",
    appearanceCategory: "Apariencia",
    languageDescription: "Elige el idioma de la interfaz.",
    themeTitle: "Tema",
    themeDescription: "Elige una apariencia predefinida para FileNavigation.",
    lightTheme: "Claro",
    darkTheme: "Oscuro",
    automaticUpdateButton: "Descargar, instalar y reiniciar",
    preparingUpdate: "Detectando el sistema y preparando la actualización...",
    updateDownloading: (percent) => "Descargando actualización... " + percent + "%",
    installingUpdate: "Descarga completada. Instalando...",
    restartingUpdate: "Instalación completada. Reiniciando...",
    automaticUpdateFailed: "La actualización automática falló.",
    updaterUnavailable: "No hay un paquete de actualización compatible con este sistema.",
    toolsLabel: "Herramientas", toolSearch: "Buscar", toolIntegrity: "Integridad",
    integritySourceLabel: "Origen", integritySourceFile: "Un archivo", integritySourceFolder: "Carpeta completa", integritySourceCurrent: "Carpeta de búsqueda actual", integritySourceFiltered: "Resultados filtrados",
    integrityTargetLabel: "Archivo o carpeta", integrityTargetPlaceholder: "Elige un archivo o carpeta", pickIntegrityTarget: "Elegir archivo o carpeta", pickIntegrityFolder: "Elegir carpeta para verificar", pickIntegrityFile: "Elegir archivo para verificar",
    integrityAlgorithmLabel: "Algoritmo", expectedHashLabel: "Huella esperada (opcional)", expectedHashPlaceholder: "Pega una huella anterior para comparar", verifyIntegrityButton: "Verificar",
    integrityEyebrow: "Integridad", integrityReady: "Listo para verificar", integrityFiles: "Archivos", integrityBytes: "Leído",
    integrityProgressIdle: "Elige un origen para calcular su huella.", integrityEmptyTitle: "Verifica archivos localmente", integrityEmptyText: "Calcula la huella de un archivo, una carpeta o los resultados filtrados.",
    integrityReading: "Calculando huellas", integrityComplete: "Verificación completada", integrityCompleteDetail: "Se leyeron todos los archivos y la huella global está lista.",
    integrityCancelled: "Verificación cancelada", integrityCancelledDetail: "La operación se detuvo antes de verificar todos los archivos.", integrityFailed: "Error de verificación",
    integrityCompleteTitle: "Huella calculada", integrityCompleteText: "Guarda la huella global para volver a verificar este conjunto.",
    integrityMatchTitle: "Integridad verificada", integrityMatchText: "La huella calculada coincide con el valor esperado.",
    integrityMismatchTitle: "Integridad no coincidente", integrityMismatchText: "El contenido, las rutas o el conjunto de archivos difieren de la huella esperada.",
    aggregateFingerprint: "Huella global", copyFingerprint: "Copiar huella", fingerprintCopied: "Huella copiada.", copyFailed: "No se pudo copiar la huella.", fileFingerprints: "Huellas de archivos",
    invalidFile: "Elige un archivo válido.", invalidExpectedHash: "La huella esperada no es válida para el algoritmo elegido.", unsupportedHashAlgorithm: "El algoritmo de hash no es compatible.",
    integrityTaskFailed: "No se pudo completar la verificación.", fileReadFailed: "No se pudo leer un archivo.", fileChangedDuringCheck: "Un archivo cambió durante la verificación. Vuelve a intentarlo.",
    folderReadFailed: "No se pudo leer toda la carpeta. No se generó ninguna huella.", noFilteredFiles: "Los resultados filtrados no contienen archivos.", invalidIntegritySource: "Elige un origen válido."
    ,toolDuplicates: "Duplicados", duplicatesSourceLabel: "Origen del análisis", duplicatesSourceFolder: "Elegir carpeta", duplicatesSourceCurrent: "Carpeta de búsqueda actual", duplicatesSourceFiltered: "Resultados filtrados",
    duplicatesFolderLabel: "Carpeta", duplicatesFolderPlaceholder: "Elige una carpeta para analizar", pickDuplicatesFolder: "Elegir carpeta para analizar", duplicatesSafetyNote: "Los archivos se comparan con SHA-256. Las copias eliminadas van a la papelera del sistema.", scanDuplicatesButton: "Buscar duplicados",
    duplicatesEyebrow: "Archivos duplicados", duplicatesReady: "Listo para analizar", duplicateGroups: "Grupos", duplicateCopies: "Copias extra", reclaimableSpace: "Recuperable", duplicatesProgressIdle: "Elige un origen para buscar archivos idénticos.",
    duplicatesEmptyTitle: "Busca duplicados de forma segura", duplicatesEmptyText: "Solo se agrupan coincidencias byte a byte. Nada se selecciona ni elimina automáticamente.", selectedCopies: "seleccionados", autoSelectDuplicates: "Seleccionar copias extra", moveSelectedToTrash: "Mover a la papelera",
    duplicatesConfirmEyebrow: "Confirmar limpieza", duplicatesConfirmTitle: "¿Mover las copias seleccionadas a la papelera?", duplicatesConfirmSafety: "Siempre se conserva un archivo verificado en cada grupo.", cancelAction: "Cancelar", confirmMoveToTrash: "Mover a la papelera", duplicatesConfirmDetail: (count, size) => `¿Mover ${count} archivos seleccionados (${size}) a la papelera del sistema?`,
    duplicatesScanning: "Buscando duplicados", duplicatesHashing: "Comparando el contenido", duplicatesComplete: "Análisis completado", duplicatesCompleteDetail: (groups, size) => `${groups} grupos encontrados · ${size} recuperables`, duplicatesNoMatches: "No se encontraron duplicados", duplicatesNoMatchesDetail: "No se encontraron archivos idénticos byte a byte en este origen.", duplicatesCancelled: "Análisis cancelado", duplicatesCancelledDetail: "El análisis se detuvo antes de comparar todos los candidatos.", duplicatesFailed: "Error al buscar duplicados",
    duplicateGroupTitle: (index) => `Grupo ${index}`, keptCopy: "Conservar", openItem: "Abrir archivo", duplicateKeepRequired: "Debe quedar al menos un archivo en cada grupo.", duplicateTaskFailed: "No se pudo completar el análisis.", invalidDuplicateSource: "Elige un origen válido.", duplicateCancelled: "El análisis de duplicados fue cancelado.", duplicateSelectionChanged: "Los archivos seleccionados cambiaron o ya no son idénticos. Analiza de nuevo.", noDuplicateSelection: "Selecciona al menos una copia duplicada.", duplicateDeleteFailed: "No se pudieron mover los archivos a la papelera.",
    duplicatesDeleteComplete: "Limpieza completada", duplicatesDeleteCompleteDetail: "Las copias seleccionadas se movieron a la papelera.", duplicatesDeletedDetail: (count, size) => `${count} archivos movidos · ${size} recuperados`, duplicatesDeletePartial: "Limpieza parcial", duplicatesDeletePartialDetail: (deleted, failed) => `${deleted} archivos movidos; ${failed} no se pudieron mover.`
};

export default messages;
