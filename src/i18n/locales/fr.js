const messages = {
    brandSubtitle: "Recherche locale de fichiers",
    minimizeWindow: "Réduire la fenêtre",
    maximizeWindow: "Agrandir ou restaurer la fenêtre",
    closeWindow: "Fermer la fenêtre",
    languageLabel: "Langue",
    rootLabel: "Dossier de recherche",
    rootPlaceholder: "Choisissez ou saisissez un chemin",
    pickDirectory: "Choisir un dossier",
    queryLabel: "Mot-clé du nom de fichier",
    queryPlaceholder: "Par exemple report, .rs, facture",
    regexPlaceholder: "Par exemple ^report_\\d{4}\\.pdf$",
    caseSensitive: "Respecter la casse",
    useRegex: "Expression régulière",
    includeHidden: "Inclure les fichiers masqués",
    searchButton: "Rechercher",
    eyebrow: "Recherche",
    waiting: "Prêt à rechercher",
    statFiles: "Fichiers analysés",
    statDirs: "Dossiers analysés",
    statTime: "Durée",
    emptyTitle: "Sélectionnez un dossier à parcourir ou saisissez un mot-clé de nom de fichier",
    emptyText: "La recherche s’exécute localement. Aucun nom, chemin ou contenu n’est envoyé.",
    emptyFolderTitle: "Le dossier est vide",
    emptyFolderText: "Aucun élément visible n’a été trouvé dans le dossier sélectionné.",
    searching: "Recherche...",
    found: (count) => `${count} résultat${count === 1 ? "" : "s"} trouvé${count === 1 ? "" : "s"}`,
    noResultsTitle: "Aucune correspondance",
    noResultsText: "Essayez un autre mot-clé ou incluez les fichiers masqués.",
    searchFailed: "Échec de la recherche",
    cannotComplete: "Impossible de terminer la recherche",
    openFailed: "Ouverture impossible",
    file: "Fichier",
    directory: "Dossier",
    expandFolder: "Développer le dossier",
    collapseFolder: "Réduire le dossier",
    unknownTime: "Heure inconnue",
    invalidDirectory: "Le dossier sélectionné n'existe pas ou n'est pas un dossier.",
    invalidRegex: "L'expression régulière n'est pas valide.",
    searchTaskFailed: "La tâche de recherche a échoué.",
    openTaskFailed: "La tâche d’ouverture a échoué.",
    openPathFailed: "Impossible d’ouvrir ce chemin.",
    updateEyebrow: "Mises à jour",
    updateTitle: "Mise à jour",
    checkUpdateButton: "Vérifier",
    checkingUpdate: "Recherche de mises à jour...",
    updateIdle: "Consulte GitHub Releases pour trouver la dernière version.",
    updateAvailable: (version) => `La version ${version} est disponible.`,
    upToDate: "Vous utilisez la dernière version.",
    currentVersion: "Actuelle",
    latestVersion: "Dernière",
    publishedAt: "Publié",
    releaseNotes: "Notes de version",
    openReleaseButton: "Ouvrir la release",
    emptyReleaseNotes: "Cette version ne contient pas de notes.",
    updateCheckFailed: "Impossible de vérifier les mises à jour.",
    versionCompareFailed: "Impossible de comparer les versions.",
    packageChecksums: "Sommes de contrôle des paquets",
    noPackageChecksums: "GitHub n'a renvoyé aucune métadonnée de contrôle des paquets.",
    updateRateLimited: "La limite d'accès à GitHub est atteinte. Réessayez plus tard.",
    updateNotFound: "Aucune version GitHub n'a été trouvée.",
    updateNetworkFailed: "La requête réseau a échoué.",
    cancelSearchButton: "Annuler",
    statSkipped: "Ignorés",
    resultFilterLabel: "Filtrer les résultats",
    resultFilterPlaceholder: "Filtrer les résultats visibles",
    typeFilterLabel: "Type",
    typeAll: "Tous",
    typeFiles: "Fichiers",
    typeFolders: "Dossiers",
    sortLabel: "Trier",
    sortNameAsc: "Nom A-Z",
    sortNameDesc: "Nom Z-A",
    sortModifiedDesc: "Plus récents",
    sortModifiedAsc: "Plus anciens",
    sortSizeDesc: "Plus grands",
    sortSizeAsc: "Plus petits",
    progressIdle: "La progression apparaîtra pendant la recherche.",
    scanningProgress: (matches, path) => "Analyse... " + matches + " correspondances" + (path ? " · " + path : ""),
    searchCancelled: "Recherche annulée",
    noVisibleResultsTitle: "Aucun résultat visible",
    noVisibleResultsText: "Ajustez les filtres ou le tri.",
    visibleResultCount: (visible, total) => "Affichage de " + visible + " résultats sur " + total,
    openUpdates: "Rechercher des mises à jour",
    openSettings: "Ouvrir les paramètres",
    closeDialog: "Fermer la fenêtre",
    preferencesEyebrow: "Préférences",
    settingsTitle: "Paramètres",
    settingsCategories: "Catégories de paramètres",
    appearanceCategory: "Apparence",
    languageDescription: "Choisissez la langue de l'interface.",
    themeTitle: "Thème",
    themeDescription: "Choisissez une apparence prédéfinie pour FileNavigation.",
    lightTheme: "Clair",
    darkTheme: "Sombre",
    automaticUpdateButton: "Télécharger, installer et redémarrer",
    preparingUpdate: "Détection du système et préparation de la mise à jour...",
    updateDownloading: (percent) => "Téléchargement de la mise à jour... " + percent + "%",
    installingUpdate: "Téléchargement terminé. Installation...",
    restartingUpdate: "Installation terminée. Redémarrage...",
    automaticUpdateFailed: "La mise à jour automatique a échoué.",
    updaterUnavailable: "Aucun paquet de mise à jour compatible avec ce système.",
    toolsLabel: "Outils", toolSearch: "Recherche", toolIntegrity: "Intégrité",
    integritySourceLabel: "Source", integritySourceFile: "Un fichier", integritySourceFolder: "Dossier entier", integritySourceCurrent: "Dossier de recherche actuel", integritySourceFiltered: "Résultats filtrés",
    integrityTargetLabel: "Fichier ou dossier", integrityTargetPlaceholder: "Choisir un fichier ou dossier", pickIntegrityTarget: "Choisir un fichier ou dossier", pickIntegrityFolder: "Choisir le dossier à vérifier", pickIntegrityFile: "Choisir le fichier à vérifier",
    integrityAlgorithmLabel: "Algorithme", expectedHashLabel: "Empreinte attendue (facultatif)", expectedHashPlaceholder: "Coller une ancienne empreinte à comparer", verifyIntegrityButton: "Vérifier",
    integrityEyebrow: "Intégrité", integrityReady: "Prêt à vérifier", integrityFiles: "Fichiers", integrityBytes: "Lus",
    integrityProgressIdle: "Choisissez une source pour calculer son empreinte.", integrityEmptyTitle: "Vérifier les fichiers localement", integrityEmptyText: "Calculez l’empreinte d’un fichier, d’un dossier ou des résultats filtrés.",
    integrityReading: "Calcul des empreintes", integrityComplete: "Vérification terminée", integrityCompleteDetail: "Tous les fichiers ont été lus et l’empreinte globale est prête.",
    integrityCancelled: "Vérification annulée", integrityCancelledDetail: "L’opération s’est arrêtée avant la vérification de tous les fichiers.", integrityFailed: "Échec de la vérification",
    integrityCompleteTitle: "Empreinte calculée", integrityCompleteText: "Conservez l’empreinte globale pour vérifier à nouveau cet ensemble.",
    integrityMatchTitle: "Intégrité vérifiée", integrityMatchText: "L’empreinte calculée correspond à la valeur attendue.",
    integrityMismatchTitle: "Intégrité différente", integrityMismatchText: "Le contenu, les chemins ou l’ensemble des fichiers diffèrent de l’empreinte attendue.",
    aggregateFingerprint: "Empreinte globale", copyFingerprint: "Copier l’empreinte", fingerprintCopied: "Empreinte copiée.", copyFailed: "Impossible de copier l’empreinte.", fileFingerprints: "Empreintes des fichiers",
    invalidFile: "Choisissez un fichier valide.", invalidExpectedHash: "L’empreinte attendue est invalide pour l’algorithme choisi.", unsupportedHashAlgorithm: "Cet algorithme de hachage n’est pas pris en charge.",
    integrityTaskFailed: "Impossible de terminer la vérification.", fileReadFailed: "Impossible de lire un fichier.", fileChangedDuringCheck: "Un fichier a changé pendant la vérification. Recommencez.",
    folderReadFailed: "Le dossier n’a pas pu être entièrement lu. Aucune empreinte n’a été créée.", noFilteredFiles: "Les résultats filtrés ne contiennent aucun fichier.", invalidIntegritySource: "Choisissez une source valide."
    ,toolDuplicates: "Doublons", duplicatesSourceLabel: "Source d’analyse", duplicatesSourceFolder: "Choisir un dossier", duplicatesSourceCurrent: "Dossier de recherche actuel", duplicatesSourceFiltered: "Résultats filtrés",
    duplicatesFolderLabel: "Dossier", duplicatesFolderPlaceholder: "Choisissez un dossier à analyser", pickDuplicatesFolder: "Choisir le dossier à analyser", duplicatesSafetyNote: "Les fichiers sont comparés par SHA-256. Les copies supprimées vont dans la corbeille système.", scanDuplicatesButton: "Rechercher les doublons",
    duplicatesEyebrow: "Fichiers en double", duplicatesReady: "Prêt à analyser", duplicateGroups: "Groupes", duplicateCopies: "Copies en trop", reclaimableSpace: "Récupérable", duplicatesProgressIdle: "Choisissez une source pour trouver les fichiers identiques.",
    duplicatesEmptyTitle: "Trouvez les doublons en sécurité", duplicatesEmptyText: "Seuls les fichiers identiques octet par octet sont groupés. Rien n’est sélectionné ni supprimé automatiquement.", selectedCopies: "sélectionnés", autoSelectDuplicates: "Sélectionner les copies en trop", moveSelectedToTrash: "Mettre à la corbeille",
    duplicatesConfirmEyebrow: "Confirmer le nettoyage", duplicatesConfirmTitle: "Mettre les copies sélectionnées à la corbeille ?", duplicatesConfirmSafety: "Un fichier vérifié est toujours conservé dans chaque groupe.", cancelAction: "Annuler", confirmMoveToTrash: "Mettre à la corbeille", duplicatesConfirmDetail: (count, size) => `Mettre ${count} fichiers sélectionnés (${size}) dans la corbeille système ?`,
    duplicatesScanning: "Recherche des doublons", duplicatesHashing: "Comparaison du contenu", duplicatesComplete: "Analyse terminée", duplicatesCompleteDetail: (groups, size) => `${groups} groupes trouvés · ${size} récupérables`, duplicatesNoMatches: "Aucun doublon trouvé", duplicatesNoMatchesDetail: "Aucun fichier identique octet par octet n’a été trouvé dans cette source.", duplicatesCancelled: "Analyse annulée", duplicatesCancelledDetail: "L’analyse s’est arrêtée avant la comparaison de tous les candidats.", duplicatesFailed: "Échec de l’analyse des doublons",
    duplicateGroupTitle: (index) => `Groupe ${index}`, keptCopy: "Conserver", openItem: "Ouvrir le fichier", duplicateKeepRequired: "Au moins un fichier doit rester dans chaque groupe.", duplicateTaskFailed: "L’analyse des doublons n’a pas pu être terminée.", invalidDuplicateSource: "Choisissez une source valide.", duplicateCancelled: "L’analyse des doublons a été annulée.", duplicateSelectionChanged: "Les fichiers sélectionnés ont changé ou ne sont plus identiques. Relancez l’analyse.", noDuplicateSelection: "Sélectionnez au moins une copie.", duplicateDeleteFailed: "Les fichiers n’ont pas pu être mis à la corbeille.",
    duplicatesDeleteComplete: "Nettoyage terminé", duplicatesDeleteCompleteDetail: "Les copies sélectionnées ont été mises à la corbeille.", duplicatesDeletedDetail: (count, size) => `${count} fichiers déplacés · ${size} récupérés`, duplicatesDeletePartial: "Nettoyage partiel", duplicatesDeletePartialDetail: (deleted, failed) => `${deleted} fichiers déplacés ; ${failed} n’ont pas pu l’être.`
};

export default messages;
