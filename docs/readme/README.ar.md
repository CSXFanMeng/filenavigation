# FileNavigation

[جميع اللغات](../../README.md) · [أحدث إصدار](https://github.com/CSXFanMeng/filenavigation/releases/latest)

FileNavigation هو تطبيق سطح مكتب مبني باستخدام Rust وTauri للبحث عن الملفات والمجلدات داخل مجلد محلي محدد. يتم البحث بالكامل على الجهاز ولا يتم رفع أسماء الملفات أو المسارات أو المحتوى.

## الميزات

- بحث محلي عن الملفات والمجلدات
- تحقق محلي باستخدام SHA-256/SHA-512 لملف واحد أو مجلد كامل أو النتائج المصفاة مع بصمة إجمالية قابلة للمقارنة
- تنظيف الملفات المكررة في المجلدات أو مجلد البحث الحالي أو النتائج المصفاة، مع تأكيد SHA-256 وحماية نسخة واحدة في كل مجموعة والاستعادة من سلة المهملات
- مطابقة أسماء الملفات حرفيا أو بالتعبير النمطي مع التحكم في حالة الأحرف
- خلفية Rust/Tauri غير متزامنة بالكامل مع الإلغاء والتقدم المباشر
- تصفية النتائج وفرزها وعرضها تدريجيا
- شجرة نتائج نسبية متعددة المستويات مع مجلدات مطوية افتراضيا وترتيب المجلدات أولا وأحجام مجمعة
- تصفح جميع العناصر الظاهرة دون كلمة مفتاحية أو حد لعدد النتائج
- واجهة حديثة متجاوبة بأيقونات Lucide وحالات تركيز سهلة الوصول
- نافذة بلا إطار بحدود يرسمها التطبيق وأزرار مترجمة
- مساحة عمل ثابتة وقائمة نتائج تمرر بشكل مستقل
- واجهة بـ20 لغة محفوظة في ملفات مستقلة
- نافذة تحديث في شريط العنوان تعرض ملاحظات GitHub المترجمة والقابلة للتمرير مع تثبيت تلقائي موقع
- نافذة إعدادات بسمات فاتحة وداكنة محفوظة
- عرض بصمات حزم التثبيت للتحقق من سلامتها

## التنزيل

- Windows: `.exe` و`.msi`
- macOS: `.dmg` و`.app`
- Linux: `.deb` و`.rpm` و`.AppImage`

تتوفر الحزم في [GitHub Releases](https://github.com/CSXFanMeng/filenavigation/releases/latest).

## التطوير والبناء

```bash
npm install
npm run tauri:dev
npm run tauri:build
```

إذا منع PowerShell تشغيل `npm.ps1` فاستخدم `npm.cmd`. توجد ملفات البناء في `src-tauri/target/release/bundle/`.

## اللغات والتحديثات

توجد الترجمات في `src/i18n/locales`. يستخدم [RELEASE_NOTES.md](../../RELEASE_NOTES.md) كتل `<!-- lang:xx -->`. يبدأ فاحص التحديث باللغة الحالية ثم الإنجليزية ثم نص Release الكامل.

## بُني باستخدام OpenAI Codex وGPT-5.6

استُخدم OpenAI Codex وGPT-5.6 كشريكين هندسيين للمساعدة في تصميم البنية، والبحث غير المتزامن القابل للإلغاء، والتدويل المعياري، وتصحيح الواجهة المتجاوبة بلا إطار، وأتمتة CI والإصدارات عبر المنصات، والتوثيق متعدد اللغات، والتحقق المستمر عبر عمليات البناء والاختبارات.

يُستخدم الذكاء الاصطناعي أثناء التطوير فقط. لا يتصل FileNavigation بخدمات OpenAI وقت التشغيل، ولا يرسل أسماء الملفات أو المسارات أو عمليات البحث أو محتويات الملفات إلى أي نموذج.

## الترخيص

يستخدم المشروع [FileNavigation Non-Commercial Source License](../../LICENSE.md). يحظر الاستخدام التجاري وإعادة البيع واستضافة SaaS والخدمات المدفوعة والتكامل التجاري المغلق والتوزيع ضمن منتجات تجارية.
