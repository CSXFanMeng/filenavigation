...
- 在指定目录下搜索本地文件和文件夹
- 使用 SHA-256/SHA-512 在本地校验单个文件、整个文件夹或当前筛选结果，并生成可比对的整体指纹
- 清理指定文件夹、当前搜索目录或筛选结果中的重复文件；使用 SHA-256 确认内容，每组强制保留一份并移入系统回收站
- 支持普通关键字或正则表达式文件名匹配，并可控制大小写
- 全异步 Rust/Tauri 后端，支持取消搜索和实时进度
- 结果过滤、类型过滤、排序及大量结果渐进渲染
- 以搜索目录为根显示多级结果树，目录默认折叠、优先排序并汇总全部子级文件大小
- 仅指定目录、不输入关键字时直接浏览目录下的全部可见内容，不限制结果数量
