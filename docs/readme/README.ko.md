...
- 로컬 파일 및 폴더 검색
- 단일 파일, 전체 폴더 및 현재 필터 결과를 SHA-256/SHA-512로 로컬 검사하고 비교 가능한 전체 지문 생성
- 폴더, 현재 검색 폴더 또는 필터 결과의 중복 파일을 SHA-256으로 확인하고 그룹마다 한 파일을 보호한 뒤 시스템 휴지통으로 이동
- 대소문자 설정을 지원하는 일반 문자열 또는 정규식 파일 이름 검색
- 취소와 실시간 진행 상황을 지원하는 완전 비동기 Rust/Tauri 백엔드
- 결과 필터링, 유형 필터, 정렬 및 점진적 렌더링
- 선택한 루트 기준 다단계 결과 트리, 기본 폴더 접기, 폴더 우선 정렬 및 폴더 합계 크기
- 키워드나 결과 수 제한 없이 선택한 폴더의 모든 표시 가능한 항목 탐색
- Lucide 아이콘과 접근 가능한 포커스 상태를 갖춘 반응형 UI
- 앱이 그린 테두리와 다국어 제어를 갖춘 프레임리스 창
- 고정 작업 공간과 독립적으로 스크롤되는 결과 목록
- 개별 언어 파일로 관리되는 20개 언어 인터페이스
- 제목 표시줄 업데이트 창에서 스크롤 가능한 다국어 GitHub 릴리스 노트를 표시하고 서명된 자동 설치 지원
- 저장되는 라이트 및 다크 테마를 제공하는 설정 창
