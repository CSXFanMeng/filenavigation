...
- ローカルのファイルとフォルダーを検索
- 単一ファイル、フォルダー全体、現在の絞り込み結果を SHA-256/SHA-512 でローカル検証し、比較可能な全体フィンガープリントを生成
- 大文字小文字を指定できる文字列または正規表現のファイル名検索
- キャンセルとリアルタイム進行状況に対応した完全非同期 Rust/Tauri バックエンド
- 結果の絞り込み、種類フィルター、並べ替え、段階的表示
- 選択ルート基準の多階層結果ツリー、フォルダーの既定折りたたみ、フォルダー優先並べ替え、合計サイズ表示
- キーワードや結果数上限なしで選択フォルダー内のすべての表示可能な項目を閲覧
