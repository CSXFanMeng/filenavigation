...
- 在指定目錄下搜尋本機檔案和資料夾
- 使用 SHA-256/SHA-512 在本機驗證單一檔案、整個資料夾或目前篩選結果，並產生可比對的整體指紋
- 清理指定資料夾、目前搜尋資料夾或篩選結果中的重複檔案；使用 SHA-256 確認內容，每組強制保留一份並移至系統垃圾桶
- 支援一般關鍵字或正規表示式檔名比對，並可控制大小寫
- 全非同步 Rust/Tauri 後端，支援取消搜尋和即時進度
- 結果篩選、類型篩選、排序及大量結果漸進式渲染
- 以搜尋目錄為根顯示多層結果樹，目錄預設摺疊、優先排序並彙總全部子層檔案大小
- 僅指定目錄、不輸入關鍵字時直接瀏覽目錄下的全部可見內容，不限制結果數量
